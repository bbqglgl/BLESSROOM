...
int get_value(void)
{
   ...
   char buf1 = 0x01;
   char buf2 = 0x80;
   *spics = 0x000000B0; //Initialization
   *spiclk = 64;
         
   printk("after setting >> spics:%d\n", *spics);
   //first cycle
   while(1) {
         if(((*spics) &(1<<18)) != 0) break; //wait for tx buffer has space.(if value == 1, tx buffer at least one bit)
   }
   *spififo = buf1; //0x01 start value send
   while(1){
      if(((*spics) & (1<<16)) != 0) break; //wait for done.
   }
   printk("end first cycle!\n");
         
   //second cycle
   while(1){
      if(((*spics) &(1<<18)) != 0) break;
   }
   *spififo = buf2; // channel 0 start
   while(1){
      if(((*spics) & (1<<16)) != 0) break;
   }
   ...
   *spififo = 0x00000000; // don't care value
   while(1) 
   {
      if(((*spics)&(1<<17)) == 0) break; //check read fifo empty
...
long light_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   switch(cmd) {
      ...
      case LIGHT_GET:
         ...
         //kbuf = get_value();
         printk("%d\n",kbuf);
         copy_to_user((const void*)arg, &kbuf, sizeof(int));
         //printk("%ln\n",&arg);
         return 2;
      default:
         return 3;
